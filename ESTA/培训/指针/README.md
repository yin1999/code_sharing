...
指针的常用运算符有两个，一个是 **&**，一个是 __\*__

首先说说 **&**，它是取地址运算符（在这里它是单目运算符，区别于按位与），使用它可以得到相应变量的储存地址。

其次说说 __\*__，它是指针运算符(或称间接访问运算符，区别于乘号)，使用它可以得到指针指向空间的实际值，也可以修改指针指向空间的值。(需要区别于定义变量时，在变量名前写的星号)
