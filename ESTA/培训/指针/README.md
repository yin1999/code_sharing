...
指针变量的值中有一个非常特殊的值: **NULL**，它**不**指向系统中的任何变量或者函数。一般，我们使用它作为一个标志(返回指针的函数没有正确执行、到达链表末尾等等)
    ...
    #define NULL ((void *)0)   //WIN32
    #define NULL 0LL           //WIN64， "0LL"表示 long long int类型的常量，其值为0
    ...                        //环境不同，其声明可能有所差异

在不同的环境中，指针变量所占用的空间大小也不同，一般的，32位操作系统中，指针变量占用4个字节；64位操作系统中(生成64位的应用程序)，指针变量占用8个字节。
